...
use std::{
    ...
    env,
    thread::{self, panicking, sleep},
    time::Duration,
    vec,
};

use crate::{
    fn_dag::{EnvFnExt, FnContainerState, FnId, RunningTask},
    mechanism::{DownCmd, MechanismImpl, ScheCmd, SimEnvObserve},
    mechanism_thread::{MechCmdDistributor, MechScheduleOnceRes},
    node::{self, EnvNodeExt, NodeId},
    request::Request,
...
impl BpBalanceScheduler {
    ...
    fn find_schedule_nodeid(&self, fnid: FnId, env: &SimEnvObserve) -> usize {
        ...
        for node_id in bplist {
            ...
            // 找出 饥饿程度 最小的
            let iter_node_starve_degree =
                self.cal_cpu_starve_degree(iter_node_resource_status, fnid, env);
    ...
    fn cal_cpu_starve_degree(
        &self,
        node_resc_state: &NodeRescState,
        fnid: FnId,
        env: &SimEnvObserve,
    ) -> f32 {
        // 先取出该函数所需要的cpu
    ...
    fn get_bplist_node_status(&self, fnid: FnId, env: &SimEnvObserve) -> BpListUpdateNodes {
        ...
        for nodeid in self.latest_nodes.get(&fnid).unwrap().iter() {
            ...
            if binpack.contains(nodeid) {
                ...
                avg_mem_use_rate += iter_node_resc_state.mem_used / iter_node_resc_state.mem_limit;
        ...
        // 把平均cpu饥饿程度打在日志上
        log::info!(
            "func_id: {}, avg_cpu_starve_degree: {}, avg_mem_use_rate: {}",
            fnid,
            avg_cpu_starve_degree,
            avg_mem_use_rate
        );
        log::info!(
            "func_id: {}, max_starve_nodeid_inbp: {}, max_score_nodeid_outbp: {}",
            fnid,
            max_starve_nodeid_inbp,
            max_score_nodeid_outbp
        );
        ...
        for &fnid in &schedule_able_fns {
            ...
            if self.binpack_map.get(&fnid).unwrap().len() == 0 {
                panic!(
                    "fnid:{}, last_nodes_len:{}",
                    fnid,
                    self.latest_nodes.get(&fnid).unwrap().len()
                );
            }
            ...
            if sche_nodeid != 9999 {
                ...
                sche_node_resc_state.resource_score = 1.0
                    / (1.0
                        + sche_node_resc_state.all_task_cnt
                        + sche_node_resc_state.mem_used / sche_node_resc_state.mem_limit);
                ...
                if self.mech_impl_sign.get(&fnid).unwrap() == &true {
                    ...
                    if (bplist_resource_status.avg_mem_use_rate > MEM_THRESHOLD_TO_ADD
                        || bplist_resource_status.avg_cpu_starve_degree > CPU_THRESHOLD_TO_ADD)
                        && bplist_resource_status.join_nodeid_outbp != 9999
                    {
                        self.update_bplist(
                            fnid,
                            bplist_resource_status.join_nodeid_outbp,
                            "add",
                            env,
                        );
                    }
            ...
            // 如果bp机制触发，则开始维护该函数的bp数组
                ...
                for nodeid in binpack_nodeids.iter() {
                    if !nodes.contains(nodeid) {
                        ...
                        log::info!(
                            "fnid:{}, 节点{}被意外缩容, 移除后binpack_len:{}",
                            func.fn_id,
                            nodeid,
                            self.binpack_map.get(&func.fn_id).unwrap().len()
                        );
                ...
                if scale_down_sign == false {
                    env.fn_containers_for_each(func.fn_id, |container| {
                        ...
                        if !binpack.contains(&container.node_id) {
                            // 如果该容器最近50帧都是空闲则缩容
                            if container.recent_frame_is_idle(50)
                                && container.req_fn_state.len() == 0
                            {
                                // 发送缩容命令
                                cmd_distributor
                ...
                // 维护binpack数组
                while bplist_resource_status.avg_cpu_starve_degree < CPU_THRESHOLD_TO_REMOVE
                    || bplist_resource_status.avg_mem_use_rate > MEM_THRESHOLD_TO_ADD
                    || bplist_resource_status.avg_cpu_starve_degree > CPU_THRESHOLD_TO_ADD
                {
                    let binpack = self.binpack_map.get(&func.fn_id).unwrap();
                    ...
                    // 退出循环逻辑
                    if bplist_resource_status.avg_cpu_starve_degree < CPU_THRESHOLD_TO_REMOVE
                        && binpack.len() == 1
                    {
                        break;
                    }
                    ...
                    if bplist_resource_status.avg_cpu_starve_degree < CPU_THRESHOLD_TO_REMOVE
                        && bplist_resource_status.expel_nodeid_inbp != 9999
                    {
                        ...
                        self.update_bplist(
                            func.fn_id,
                            bplist_resource_status.expel_nodeid_inbp,
                            "remove",
                            env,
                        );
                    ...
                    else if (bplist_resource_status.avg_mem_use_rate > MEM_THRESHOLD_TO_ADD
                        || bplist_resource_status.avg_cpu_starve_degree > CPU_THRESHOLD_TO_ADD)
                        && bplist_resource_status.join_nodeid_outbp != 9999
                    {
                        self.update_bplist(
                ...
                log::info!(
                    "fnid:{}, binpack_len:{}, latest_nodes_len:{}",
                    func.fn_id,
                    binpack.len(),
                    self.latest_nodes.get(&func.fn_id).unwrap().len()
                );
            }
